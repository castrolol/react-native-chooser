...
import { View, Text, StyleSheet, TouchableWithoutFeedback, Modal, Dimensions, ViewPropTypes } from 'react-native';
...
export default class Select extends Component {
	...
	componentDidMount() {
		if (this.props.selected) {
			React.Children.map(this.props.children, (item, key) => {
				...
				if (item.props.value == this.props.selected) {
					this.onSetSelect(item.props.children, item.props.value);
				}
			});
		}
	}
	...
	componentWillReceiveProps(nextProps) {
		if (nextProps.selected == null) {
			...
			if (nextProps.selected != this.props.selected) {
				React.Children.map(nextProps.children, (item, key) => {
					if (!item) return null;

					if (item.props.value == nextProps.selected) {
						this.onSetSelect(item.props.children, item.props.value);
					}
				});
			}
	...
	onSelect(label, value) {
		...
		this.onSetSelect(label, value);
	}

	onSetSelect(label, value) {
		this.setState({
			modalVisible: false,
			defaultText: label
		});
